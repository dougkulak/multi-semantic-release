...
function gitInitOrigin(cwd, releaseBranch = null) {
	...
	// Set up a release branch. Return to main afterwards.
	if (releaseBranch) {
		execa.sync("git", ["checkout", "-b", releaseBranch], { cwd });
		execa.sync("git", ["checkout", "main"], { cwd });
	}
 ...
 */
function gitPush(cwd, remote = "origin", branch = "main") {
	// Check params.
	check(cwd, "cwd: absolute");
